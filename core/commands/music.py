...
class Music(commands.Cog, description="Jamming out with these!"):
    ...
    async def start_nodes(self):
        await self.pomice.create_node(bot=self.bot, host="disbotlistlavalink.ml", port="443", password="LAVA", identifier="LAVA", spotify_client_id=os.getenv("SPOTIFY").split(", ")[0], spotify_client_secret=os.getenv("SPOTIFY").split(", ")[1])
