...
class Music(commands.Cog, description="Jamming out with these!"):
    ...
    async def create_node_pomice(self):
        await self.bot.pomice.create_node(bot=self.bot, host="lavalink.darrennathanael.com", port="2095", password="clover", identifier="Pomice", spotify_client_id=os.getenv("SPOTIFY").split(", ")[0], spotify_client_secret=os.getenv("SPOTIFY").split(", ")[1])
