...
class ViewVote(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        if self.vote != self.voters: return await self.message.edit(content=F"Voting for {self.usage} has been ended\nVotes didn't reach the needed amount {self.voters}", view=self.view)
    ...
    async def skip(self, ctx:commands.Context):
        if ctx.voice_client.is_playing:
            if not ctx.voice_client.queue.empty():
                ctx.voice_client.loop = None
                return await ViewVote(ctx, F"Skipping: {ctx.voice_client.current.title} | {ctx.voice_client.current.author}", ctx.voice_client.stop, F"Skipped: {ctx.voice_client.current.title} | {ctx.voice_client.current.author}", len(ctx.me.voice.channel.members)).start()
