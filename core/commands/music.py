...
class Music(commands.Cog, description="Jamming out with these!"):
    ...
    async def start_nodes(self):
        await self.pomice.create_node(bot=self.bot, host="lavalink.cjstevenson.xyz", port="443", password="lookbehindyou", identifier="MAIN", spotify_client_id=os.getenv("SPOTIFY").split(", ")[0], spotify_client_secret=os.getenv("SPOTIFY").split(", ")[1])
